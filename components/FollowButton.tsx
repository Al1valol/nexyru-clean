...
export default function FollowButton({ targetUsername, currentUserId, size="md", variant="primary", showCount=false, onFollowChange }: Props) {
 ...
 const [targetId, setTargetId] = useState<string|null>(null);
  ...
  const style: any = isFollowing ? { ...base, background:isHovered?"rgba(248,113,113,0.12)":"rgba(52,211,153,0.1)", color:isHovered?"#ef4444":"#10b981", border:`1px solid ${isHovered?"rgba(248,113,113,0.3)":"rgba(52,211,153,0.25)"}` } : variant==="outline" ? { ...base, background:"transparent", color:"#6366f1", border:"1px solid rgba(99,102,241,0.4)" } : { ...base, background:"#6366f1", color:"#fff", boxShadow:"0 4px 12px rgba(99,102,241,0.25)" };
  return (
    <div style={{ display:"inline-flex", flexDirection:"column", alignItems:"center", gap:4 }}><button onClick={handleClick} disabled={!!(isLoading&&!isFollowing)} onMouseEnter={()=>setIsHovered(true)} onMouseLeave={()=>setIsHovered(false)} style={style}>
      ...
      {showCount && <span style={{ fontSize:10, color:"#2a2a3a", fontFamily:"monospace" }}>{followerCount.toLocaleString()} followers</span>}
      {error && <span style={{ fontSize:10, color:"#ef4444" }}>{error}</span>}
      <style>{`@keyframes fbspin{to{transform:rotate(360deg)}}`}</style></div>
  );
}
