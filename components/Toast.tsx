...
const TOAST_COLORS: Record<ToastType, { bg: string; border: string; text: string; icon: string }> = {
  success: { bg: "rgba(52,211,153,0.08)",  border: "#34d399", text: "#34d399", icon: "✓" },
  error:   { bg: "rgba(248,113,113,0.08)", border: "#f87171", text: "#f87171", icon: "✕" },
  warning: { bg: "rgba(251,191,36,0.08)",  border: "#fbbf24", text: "#fbbf24", icon: "⚠" },
  info:    { bg: "rgba(56,189,248,0.08)",  border: "#38bdf8", text: "#38bdf8", icon: "i" },
};
        ...
        {toasts.map(t => {
          ...
          return (
            <div key={t.id} className={`nexyru-toast${t.exiting ? " exiting" : ""}`} style={{
              ...
              <div style={{ flex:1, lineHeight:1.4, paddingTop:1 }}>{t.message}</div>
              <button onClick={() => dismiss(t.id)} style={{
                background:"transparent", border:"none", cursor:"pointer",
                color:"#475569", padding:0, fontSize:14, lineHeight:1,
