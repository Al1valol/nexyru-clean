...
const inp: React.CSSProperties = {
  ...
  background: "#111118", border: "1px solid #2a2a3a", fontSize: 13, color: "#ffffff",
...
export default function ChecklistPage() {
 ...
 return (<div style={{ display:"flex", minHeight:"100vh", background:"#0a0a0f" }}><Sidebar activePath="/checklist" /><MobileNav activePath="/checklist" /><main style={{ flex:1, marginLeft:56, minHeight: "100vh", background: "#070b14", color: "#ffffff", paddingBottom:72, fontFamily: "system-ui, -apple-system, sans-serif" }}><style>{`
      ...
      {/* Top nav */}
      <div className="checklist-header" style={{ borderBottom: "1px solid #111118", background: "rgba(6,13,26,0.95)", padding: "14px 28px", display: "flex", alignItems: "center", gap: 16, position: "sticky", top: 0, zIndex: 10, backdropFilter: "blur(8px)" }}><a href="/dashboard" style={{ fontSize: 12, color: "#2a2a3a", textDecoration: "none" }}>← Dashboard</a><span style={{ fontSize: 14, fontWeight: 800, color: "#ffffff" }}>Pre-Trade Checklist</span><div style={{ flex: 1 }} /><span style={{ fontSize: 10, color: "#2a2a3a", fontWeight: 600 }}>{username ? `@${username}` : ""}</span></div><div className="checklist-page" style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 20px" }}>

        {/* HEADER */}
        <header style={{ marginBottom: 28 }}><h1 className="checklist-title" style={{ fontSize: 30, fontWeight: 900, margin: 0, letterSpacing: "-0.02em" }}>Pre-Trade Checklist</h1><p style={{ fontSize: 13, color: "#6b7280", margin: "6px 0 0" }}>Run this before every trade</p></header>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 380px) minmax(0, 1fr)", gap: 20, alignItems: "flex-start", marginBottom: 24 }} className="checklist-top-grid">
          ...
          <section style={card}>
            {sectionTitle("#a5b4fc", "My Checklist")}
            <p style={{ margin: "0 0 14px", fontSize: 11, color: "#6b7280", lineHeight: 1.5 }}>Click an item to edit. Drag to reorder. Toggle category between Must Pass and Good to Check.</p><div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {items.map(it => {
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      background: "#111118",
                      border: `1px solid ${isMust ? "rgba(248,113,113,0.35)" : "rgba(245,158,11,0.30)"}`,
                      borderLeft: `3px solid ${isMust ? "#ef4444" : "#f59e0b"}`,
                  ...
                  ><span style={{ color: "#6b7280", fontSize: 12, cursor: "grab" }}>⋮⋮</span>
                    ...
                    {isEditing ? (
                      ...
                      />) : (<button
                        ...
                        style={{ flex: 1, textAlign: "left", background: "transparent", border: "none", color: "#ffffff", fontSize: 12.5, fontWeight: 600, cursor: "text", padding: 0 }}
                      >
                        {it.text}
                      </button>
                    )}
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: isMust ? "rgba(248,113,113,0.12)" : "rgba(245,158,11,0.12)",
                        color: isMust ? "#ef4444" : "#f59e0b",
                        border: `1px solid ${isMust ? "rgba(248,113,113,0.4)" : "rgba(245,158,11,0.4)"}`,
                      ...
                      title="Delete"
                      style={{ padding: "3px 7px", borderRadius: 6, fontSize: 13, color: "#6b7280", background: "transparent", border: "none", cursor: "pointer" }}>×</button></div>
                );
              })}
            </div><div style={{ display: "flex", gap: 8, marginTop: 14 }}><button
                onClick={addItem}
                style={{ flex: 1, padding: "10px 12px", borderRadius: 9, background: "rgba(99,102,241,0.10)", color: "#6366f1", border: "1px dashed rgba(99,102,241,0.45)", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>+ Add Item</button>
              {confirmingReset ? (
                <div style={{ display:"flex", gap:6 }}><button
                    onClick={() => { setConfirmingReset(false); resetDefaults(); }}
                    style={{ padding: "10px 12px", borderRadius: 9, background: "rgba(239,68,68,0.15)", color: "#ef4444", border: "1px solid #ef4444", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>✓ Reset</button><button
                    onClick={() => setConfirmingReset(false)}
                    style={{ padding: "10px 12px", borderRadius: 9, background: "transparent", color: "#9ca3af", border: "1px solid #374151", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>✗</button></div>) : (<button
                  onClick={() => { setConfirmingReset(true); setTimeout(() => setConfirmingReset(curr => curr ? false : curr), 3000); }}
                  title="Reset to defaults"
                  style={{ padding: "10px 12px", borderRadius: 9, background: "transparent", color: "#6b7280", border: "1px solid #2a2a3a", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Reset</button>
              )}
            </div><div style={{ marginTop: 14, padding: "10px 12px", borderRadius: 9, background: "#111118", border: "1px solid #2a2a3a", fontSize: 11, color: "#6b7280", lineHeight: 1.5 }}><span style={{ color: "#ef4444", fontWeight: 800 }}>Must Pass</span>blocks the trade if NO.<br/><span style={{ color: "#f59e0b", fontWeight: 800 }}>Good to Check</span>shows a warning only.</div></section>
          ...
          <section style={card}>
            ...
            {!running && !resultRun && (
              <><div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr 1fr 1fr", gap: 10, marginBottom: 16 }} className="trade-detail-row"><div><label style={lbl}>Symbol</label><input
                      style={inp}
                      value={symbol}
                      onChange={e => setSymbol(e.target.value.toUpperCase())}
                      placeholder="NQ1!, BTC/USD"
                    /></div><div><label style={lbl}>Direction</label><div style={{ display: "flex", gap: 6 }}>
                      {(["LONG", "SHORT"] as const).map(d => {
                        const active = direction === d;
                        const color = d === "LONG" ? "#22d3a5" : "#ef4444";
                        return (
                          <button
                            key={d}
                            ...
                            style={{
                              ...
                              background: active ? `${color}1f` : "#111118",
                    ...
                    </div></div><div><label style={lbl}>Entry</label><input style={inp} type="number" step="any" value={entry} onChange={e => setEntry(e.target.value)} placeholder="0.00"/></div><div><label style={lbl}>Stop</label><input style={inp} type="number" step="any" value={stop} onChange={e => setStop(e.target.value)} placeholder="0.00"/></div><div><label style={lbl}>Target</label><input style={inp} type="number" step="any" value={target} onChange={e => setTarget(e.target.value)} placeholder="0.00"/></div></div><div className="checklist-context-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 18 }}><StatTile label="R:R" value={calcs.rr !== null ? `${calcs.rr}R` : "—"} color={calcs.rr !== null && calcs.rr >= 2 ? "#22d3a5" : calcs.rr !== null && calcs.rr >= 1 ? "#f59e0b" : "#6b7280"} /><StatTile label="Dollar risk" value={fmtMoney0(calcs.dollarRisk)} color="#f59e0b" /><StatTile label="Contracts" value={calcs.contracts !== null ? String(calcs.contracts) : "—"} color="#6366f1" /></div><button
                  onClick={beginRun}
                  className="pulse"
                  style={{
                ...
                </button><p style={{ marginTop: 10, fontSize: 11, color: "#6b7280", textAlign: "center" }}>Trade details are optional — but help you stay focused.</p></>
            )}
            ...
            {running && currentItem && (
              <div>
                {/* Progress */}
                <div style={{ marginBottom: 18 }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><span style={{ fontSize: 11, fontWeight: 800, color: "#9ca3af", letterSpacing: "0.06em" }}>
                      {stepIndex + 1} / {totalSteps}
                    </span><span style={{ fontSize: 11, fontWeight: 700, color: "#6b7280" }}>
                      {currentItem.category === "must"
                        ? <span style={{ color: "#ef4444" }}>Must Pass</span>:<span style={{ color: "#f59e0b" }}>Good to Check</span>}
                    </span></div><div style={{ height: 8, background: "#2a2a3a", borderRadius: 4, overflow: "hidden" }}><div style={{
                ...
                <div
                  ...
                  style={{
                    ...
                    border: `2px solid ${currentItem.category === "must" ? "rgba(248,113,113,0.35)" : "rgba(245,158,11,0.30)"}`,
                    borderRadius: 18,
                    padding: "44px 28px",
                    textAlign: "center",
                    minHeight: 240,
                    ...
                    marginBottom: 16,
                  }}><div style={{ fontSize: 22, fontWeight: 800, color: "#ffffff", lineHeight: 1.35, maxWidth: 560, margin: "0 auto" }}>
                    {currentItem.text}
                  </div></div>

                {/* YES / NO */}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}><button
                    onClick={() => answer("yes")}
                    className="yesno-btn"
                    style={{
                    ...
                    style={{
                      ...
                      transition: "transform 0.12s",
                    }}>NO</button></div><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><button onClick={goBack} style={{ padding: "8px 14px", background: "transparent", color: "#6b7280", border: "1px solid #2a2a3a", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>← Back</button><button onClick={cancelRun} style={{ padding: "8px 14px", background: "transparent", color: "#6b7280", border: "none", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Cancel</button></div></div>
            )}
        ...
        <section style={{ ...card, marginBottom: 40 }}>
          {sectionTitle("#6366f1", "Checklist Stats")}

          {runs.length === 0 ? (
            <div style={{ background: "#111118", border: "1px dashed #2a2a3a", borderRadius: 12, padding: "36px 20px", textAlign: "center" }}><div style={{ fontSize: 30, marginBottom: 8 }}></div><p style={{ margin: 0, color: "#6b7280", fontSize: 13 }}>Run the checklist before your next trade to start tracking discipline.</p></div>) : (<div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}><StatTile label="Total runs" value={String(stats.total)} color="#ffffff" /><StatTile label="Pass rate" value={`${stats.passRate}%`} color={stats.passRate >= 70 ? "#22d3a5" : stats.passRate >= 40 ? "#f59e0b" : "#ef4444"} /><StatTile
...
function ResultPanel({
  ...
  if (run.passed) {
    return (
      <div className="card-enter" style={{ textAlign: "center" }}><div style={{
        ...
        }}><div style={{ fontSize: 48, marginBottom: 8 }}></div><h2 style={{ margin: 0, fontSize: 28, fontWeight: 900, color: "#22d3a5", letterSpacing: "-0.02em" }}>GO FOR IT</h2><p style={{ margin: "8px 0 0", fontSize: 13, color: "#9ca3af" }}>All Must Pass items checked. You're cleared.</p></div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10, marginBottom: 16 }}>
          {run.symbol && <StatTile label="Symbol" value={run.symbol} color="#ffffff" />}
          {run.direction && <StatTile label="Direction" value={run.direction} color={run.direction === "LONG" ? "#22d3a5" : "#ef4444"} />}
          {run.entry !== undefined && <StatTile label="Entry" value={String(run.entry)} color="#6366f1" />}
          {run.stop !== undefined && <StatTile label="Stop" value={String(run.stop)} color="#ef4444" />}
          {run.target !== undefined && <StatTile label="Target" value={String(run.target)} color="#22d3a5" />}
          {run.rr !== null && run.rr !== undefined && <StatTile label="R:R" value={`${run.rr}R`} color={run.rr >= 2 ? "#22d3a5" : "#f59e0b"} />}
          {run.dollarRisk !== null && run.dollarRisk !== undefined && <StatTile label="Risk" value={fmtMoney0(run.dollarRisk)} color="#f59e0b" />}
          {run.contracts !== null && run.contracts !== undefined && <StatTile label="Contracts" value={String(run.contracts)} color="#6366f1" />}
        </div><div style={{ display: "flex", gap: 10 }}><button
            onClick={onLog}
            style={{
              ...
              boxShadow: "0 6px 20px rgba(34,211,165,0.30)",
            }}>Log This Trade</button><button
            onClick={onRunAgain}
            style={{ padding: "16px 20px", borderRadius: 12, background: "transparent", color: "#6b7280", border: "1px solid #2a2a3a", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Run Again</button></div></div>
    );
  }

  // FAIL
  const failed = run.items.filter(i =>i.category === "must" && i.answer === "no");
 return (<div className="card-enter" style={{ textAlign: "center" }}><div style={{
        background: "linear-gradient(135deg, rgba(248,113,113,0.18), rgba(248,113,113,0.04))",
        border: "1px solid rgba(248,113,113,0.45)",
        borderRadius: 18, padding: "30px 22px", marginBottom: 16,
      }}><div style={{ fontSize: 48, marginBottom: 8 }}></div><h2 style={{ margin: 0, fontSize: 26, fontWeight: 900, color: "#ef4444", letterSpacing: "-0.02em" }}>DON'T TAKE THIS TRADE</h2><p style={{ margin: "8px 0 0", fontSize: 13, color: "#9ca3af" }}>Sit this one out. The market will be here tomorrow — your account might not.</p></div>

      {failed.length >0 && (<div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 12, padding: 14, marginBottom: 14, textAlign: "left" }}><div style={{ fontSize: 10, fontWeight: 800, color: "#ef4444", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 8 }}>What failed</div><div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {failed.map(f =>(<div key={f.itemId} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, color: "#ffffff" }}><span style={{ color: "#ef4444", fontWeight: 800 }}>✗</span>
                {f.text}
              </div>
            ))}
          </div></div>
      ...
      <div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 12, padding: 14, marginBottom: 14, textAlign: "left" }}><div style={{ fontSize: 10, fontWeight: 800, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 8 }}>What went wrong?</div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 6 }}>
          {FAIL_REASONS.map(r => {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: active ? "rgba(248,113,113,0.15)" : "#111118",
                  color: active ? "#ef4444" : "#9ca3af",
                  border: `1px solid ${active ? "rgba(248,113,113,0.5)" : "#2a2a3a"}`,
        ...
        </div></div><div style={{ display: "flex", gap: 10 }}><button
          onClick={onSitOut}
          style={{
            flex: 1, padding: "16px 20px", borderRadius: 12,
            background: "linear-gradient(135deg, #2a2a3a, #111118)", color: "#ffffff",
            border: "1px solid #374151", fontSize: 14, fontWeight: 800, cursor: "pointer",
          }}>Sit this one out</button><button
          onClick={onRunAgain}
          style={{ padding: "16px 20px", borderRadius: 12, background: "transparent", color: "#6b7280", border: "1px solid #2a2a3a", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Run Again</button></div></div>
  );
}
...
function StatTile({ label, value, color, sub }: { label: string; value: string; color: string; sub?: string }) {
  return (
    <div style={cardSm}><div style={{ fontSize: 10, color: "#6b7280", fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 6 }}>{label}</div><div style={{ fontSize: 20, fontWeight: 900, color, fontFamily: "system-ui", lineHeight: 1.2, wordBreak: "break-word" }}>{value}</div>
      {sub && <div style={{ fontSize: 10.5, color: "#6b7280", marginTop: 6, fontWeight: 600 }}>{sub}</div>}
    </div>
  );
}
