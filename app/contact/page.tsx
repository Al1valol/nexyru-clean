            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              {[{label:"Name",value:name,set:setName,type:"text",placeholder:"Your name"},{label:"Email",value:email,set:setEmail,type:"email",placeholder:"your@email.com"}].map(f=>(
                <div key={f.label}>
                  ...
                  <input type={f.type} value={f.value} onChange={e=>f.set(e.target.value)} placeholder={f.placeholder} style={{width:"100%",padding:"11px 14px",borderRadius:10,border:"1px solid #1a2540",background:"#0b1120",color:"#e2e8f0",fontSize:14,outline:"none",boxSizing:"border-box" as any}}/>
                </div>
              ))}
              <div>
                ...
                <textarea value={message} onChange={e=>setMessage(e.target.value)} placeholder="Tell us what's on your mind…" rows={5} style={{width:"100%",padding:"11px 14px",borderRadius:10,border:"1px solid #1a2540",background:"#0b1120",color:"#e2e8f0",fontSize:14,outline:"none",resize:"vertical",boxSizing:"border-box" as any,fontFamily:"system-ui"}}/>
              </div>
              {error&&<div style={{padding:"10px 14px",borderRadius:10,background:"rgba(248,113,113,0.08)",border:"1px solid rgba(248,113,113,0.2)",fontSize:12,color:"#f87171"}}>{error}</div>}
              <button onClick={handleSend} disabled={!ready||sending} style={{padding:"12px",borderRadius:12,border:"none",background:ready&&!sending?"linear-gradient(135deg,#0369a1,#38bdf8)":"#1a2540",color:ready&&!sending?"#fff":"#334155",fontSize:14,fontWeight:700,cursor:ready&&!sending?"pointer":"not-allowed"}}>
