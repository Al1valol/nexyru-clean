...
function getRank(s: ProfileStats) {
  if (s.totalTrades>=200&&s.winRate>=60&&s.consistency>=80) return {label:"Funded Trader",    color:"#f59e0b",next:"Max rank reached",  progress:100};
  ...
  if (s.totalTrades>=50 &&s.winRate>=50&&s.consistency>=55) return {label:"Consistent Trader",color:"#34d399",next:"Verified Trader",   progress:Math.min(100,((s.totalTrades-50)/50)*100)};
  if (s.totalTrades>=20)                                    return {label:"Active Trader",    color:"#38bdf8",next:"Consistent Trader", progress:Math.min(100,((s.totalTrades-20)/30)*100)};
...
function getBadges(s: ProfileStats, strats: Strategy[]): Badge[] {
  ...
  if (s.consistency>=75)  b.push({id:"con",  label:"Consistent",       icon:"📊",color:"#38bdf8",desc:"75+ consistency score"});
  if (s.totalTrades>=100) b.push({id:"100",  label:"Centurion",        icon:"💯",color:"#a78bfa",desc:"100+ trades"});
  if (s.maxDrawdown<10)   b.push({id:"dd",   label:"Risk Master",      icon:"🛡️",color:"#22d3a5",desc:"Max DD under 10%"});
  ...
  if (strats.some(x=>x.status==="verified"))          b.push({id:"ver", label:"Verified Strategy",icon:"✓", color:"#a78bfa",desc:"Has verified strategy"});
  if (strats.some(x=>(x.monthly_price??0)>0))         b.push({id:"earn",label:"Earning",           icon:"💰",color:"#fbbf24",desc:"Has paid subscribers"});
  return b;
}
...
export default function TraderProfile() {
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      try {
        ...
        if (raw.length > 0) {
          ...
          for (let i=1;i<days.length;i++) {
            ...
            if (diff===1){dayStreak++;if(dayStreak>bestDay)bestDay=dayStreak;}else dayStreak=1;
          }
      ...
      if (total===0) {
        ...
        winRate=Math.round(avgWr); maxDD=parseFloat(avgDD.toFixed(1));
        bestStreak=Math.floor(avgWr/15);
      }

      setStats({
        totalTrades:total, verifiedTrades, winRate,
        ...
        bestStreak, currentWinStreak, currentTradeStreak, bestTradeStreak,
  ...
  if (loading) return (
    <div style={{minHeight:"100vh",background:"#060d1a",display:"flex",alignItems:"center",justifyContent:"center",color:"#3a4a6a",fontSize:14,gap:12}}>
  ...
  const RANKS = ["Beginner","Active Trader","Consistent Trader","Verified Trader","Funded Trader"];
    ...
    <div style={{minHeight:"100vh",background:"#060d1a",color:"#c8d8f0",fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"32px 24px",animation:"fadeIn 0.4s ease"}}>
          ...
          <div style={{display:"flex",alignItems:"flex-start",gap:24,flexWrap:"wrap"}}>
            ...
            <div style={{flex:1,minWidth:200}}>
              <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",marginBottom:6}}>
                ...
                <span style={{fontSize:11,fontWeight:800,padding:"4px 12px",borderRadius:20,background:`${rank.color}18`,border:`1px solid ${rank.color}44`,color:rank.color}}>{rank.label}</span>
              </div>
              ...
              {currentUser&&currentUser!==username&&(
                <button onClick={toggleFollow} disabled={followLoading} style={{display:"inline-flex",alignItems:"center",gap:6,marginBottom:14,padding:"7px 16px",borderRadius:10,fontSize:12,fontWeight:700,cursor:followLoading?"not-allowed":"pointer",transition:"all 0.15s",border:`1px solid ${isFollowing?"rgba(248,113,113,0.3)":"rgba(56,189,248,0.3)"}`,background:isFollowing?"rgba(248,113,113,0.06)":"rgba(56,189,248,0.08)",color:isFollowing?"#f87171":"#38bdf8"}}>
              ...
              {rank.progress<100&&(
                <div>
                  ...
                  <div style={{height:5,borderRadius:3,background:"#111d30",overflow:"hidden"}}>
                    <div style={{width:`${rank.progress}%`,height:"100%",background:`linear-gradient(90deg,${rank.color}88,${rank.color})`,borderRadius:3}}/>
                  </div>
                </div>
              )}
            </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:10,marginBottom:20}}>
          ...
          <StatPill label="Consistency"  value={String(stats.consistency)} color={stats.consistency>=70?"#38bdf8":"#94a3b8"}/>
          <StatPill label="Avg RR"       value={String(stats.avgRR)}       color={stats.avgRR>=1.5?"#34d399":"#94a3b8"}/>
        ...
        <div style={{background:"#0b1120",border:"1px solid #1a2540",borderRadius:20,padding:"20px 22px",marginBottom:20}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12}}>
            {[
              {label:"Current Win Streak",val:stats.currentWinStreak,color:stats.currentWinStreak>=5?"#f97316":stats.currentWinStreak>=3?"#34d399":"#3a4a6a",emoji:stats.currentWinStreak>=10?"🔥":stats.currentWinStreak>=5?"⚡":stats.currentWinStreak>=3?"📈":"➖",hot:stats.currentWinStreak>=5},
              {label:"Best Win Streak",   val:stats.bestStreak,      color:"#fbbf24",emoji:"🏆",hot:false},
              {label:"Day Trade Streak",  val:stats.currentTradeStreak,color:stats.currentTradeStreak>=7?"#38bdf8":stats.currentTradeStreak>=3?"#94a3b8":"#3a4a6a",emoji:"📅",hot:false},
              {label:"Best Day Streak",   val:stats.bestTradeStreak, color:"#a78bfa",emoji:"⭐",hot:false},
            ].map((s,i)=>(
              <div key={i} style={{background:s.hot?"rgba(249,115,22,0.08)":"#0d1628",border:`1px solid ${s.hot?"rgba(249,115,22,0.4)":"#1a2540"}`,borderRadius:14,padding:"16px",textAlign:"center"}}>
                <div style={{fontSize:28,marginBottom:6}}>{s.emoji}</div>
                <div style={{fontSize:32,fontWeight:900,fontFamily:"monospace",color:s.color,lineHeight:1}}>{s.val}</div>
                <div style={{fontSize:10,color:"#3a4a6a",marginTop:6,fontWeight:600}}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {badges.length>0&&(
          <div style={{background:"#0b1120",border:"1px solid #1a2540",borderRadius:20,padding:"20px 22px",marginBottom:20}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
              {badges.map(b=>(
                <div key={b.id} title={b.desc} style={{display:"flex",alignItems:"center",gap:7,padding:"7px 14px",borderRadius:12,background:`${b.color}10`,border:`1px solid ${b.color}33`}}>
              ...
              <div key={s.id} style={{padding:"16px 22px",borderBottom:"1px solid #0d1828",display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
                <div style={{flex:1,minWidth:160}}>
                  <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:4}}>
                    ...
                    <span style={{fontSize:9,padding:"1px 7px",borderRadius:10,background:`${c}15`,color:c,border:`1px solid ${c}30`,fontWeight:700}}>{s.status}</span>
                  </div>
                  {bt&&<div style={{display:"flex",gap:12,fontSize:11,color:"#3a4a6a"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:14,flexShrink:0}}>
                  {(followerCounts[s.id]??0)>0&&<div style={{textAlign:"right"}}><div style={{fontSize:9,color:"#3a4a6a"}}>Followers</div><div style={{fontSize:14,fontWeight:700,color:"#38bdf8",fontFamily:"monospace"}}>{followerCounts[s.id]}</div></div>}
                  {(s.monthly_price??0)>0&&<div style={{textAlign:"right"}}><div style={{fontSize:9,color:"#3a4a6a"}}>Price</div><div style={{fontSize:14,fontWeight:700,color:"#fbbf24",fontFamily:"monospace"}}>${s.monthly_price}/mo</div></div>}
            ...
            {[
              ...
              return(
                <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",borderRadius:10,background:active?`${r.color}10`:"transparent",border:`1px solid ${active?r.color+"40":"#1a2540"}`}}>
                  <div style={{width:22,height:22,borderRadius:11,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800,background:active||past?`${r.color}20`:"#111d30",border:`1px solid ${active||past?r.color:"#1e2f4a"}`,color:active||past?r.color:"#2e3f5a"}}>
                    {past?"✓":active?"●":"○"}
                  </div>
                  <div style={{flex:1}}>
